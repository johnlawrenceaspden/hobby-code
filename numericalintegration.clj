...
;; We wish to evaluate the integral of a function over an interval
...
;; We can make them both more accurate by sampling the function at more points, and
;; dividing the sum by an appropriate factor

(/ (reduce + (map square [0 1/2 1 3/2])) 2) ; 7/4
(/ (reduce + (map square [1/2 1 3/2 1])) 2) ; 9/4

;; It's clear that these values are getting closer to one another
...
;; And we'll find that these two sums approach each other more and more closely
...
(map (partial riemann-sum square 0 2) (iterate inc 1)) ; (1 7/4 55/27 35/16 57/25 253/108 ...)
