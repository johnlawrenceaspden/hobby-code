...
;; And another would be to add the value at 1 to the value at 2
...
;; We can make both answers more accurate by sampling the function at more points, and
...
;; We can continue the procedure, splitting the interval into more and more
;; bits, and dividing by larger and larger factors to make up:
...
(take 10
      (map (partial - 8/3)
           (map (partial riemann-lower-sum square 0 2) (iterate (partial * 2) 1))))
;; (5/3 11/12 23/48 47/192 95/768 191/3072 383/12288 767/49152 1535/196608 3071/786432)



;; Strictly speaking, the riemann lower sum is a sum of the lowest values of the
;; function in the sub-intervals, but since square is a function that gets
;; bigger as x gets larger, the value calculated above is correct.
...
;; By taking the values of this antiderivative at the start and end of the interval
;; and getting the difference, we can work out what the integral is exactly:
...
;; A better estimate of this area under the graph is to imagine a trapezium with
;; its base at (a,0) and (b,0), and the other corners at (a,f(a)) (b, f(b)), and
;; calculate the area of that.
...
;; That's equivalent to taking a weighted sum of the values of f at the beginning, midpoint, and end
;; of the interval, with weights 1:4:1
