...
;;That's about it for the Sieve of Eratosthenes over a finite list, but what about producing an infinite list of primes?
...
;;The multiple of 2 is lower than this, so add 2 to get 4
;;Now our multiple is higher, so we know 3 is prime
;;The first multiple of 3 is 3
;;Up the counter to 4.
;;4 is larger than our lowest prime multiple, so increase it, by adding its prime.
...
;;The lowest multiple is 4, a multiple of the prime 2. 2+4=6
;;5 is lower than any multiple, so it is a prime too.
;;And so on........

;;In a notation I hope is obvious, the iteration goes:
...
(def sieves (iterate sieve [(sorted-set [2,2]) 3]))
...
;;and in fact it might be better if we sort them!
(sort (map second (first (nth sieves 200))))

;;again we can construct an infinite list
...
;;On my netbook, that took nearly 30 seconds.
...
(sieve (sorted-set [2,2]) 2 10)
;;now we can ask for all the primes up to 100.
(map second (sieve (sorted-set [2,2]) 2 100))
...
(take 10 (reverse (map second (sieve (sorted-set [2,2]) 2 10000))))
;;This is how long it takes:
(time (take 10 (reverse (map second (sieve (sorted-set [2,2]) 2 10000)))))
