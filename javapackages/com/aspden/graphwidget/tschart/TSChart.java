...
public class TSChart extends Panel implements ArrayRangeSelectionListener
{
	 ...
	 * A null operation, but we provide an object to generate the labels on the x axis.
	 */
	public void showRawData(ArrayLabelGenerator alg)
	{
		...
		while(e.hasMoreElements())
		{
			...
			if(t.data.length==0) continue; //someone may have added a null array, which will want its label displaying, but there's no point adding it to the graph.
	 ...
	 * This is because we need labels for points which do not exist in the unexpanded axis.
         * This function is useful if you have data say for every weekday but want to see it on a 
         * 'real' timescale. You tell the graph to put in the Saturdays and Sundays with the 
         * TSExpander, and you label the seven day week scale which results with the ArrayLabelGenerator.
	 */
	public void showExpandedData(TSExpander tse, ArrayLabelGenerator alg)
	{
		...
		for(Enumeration e = theTSData.elements();e.hasMoreElements();)
		{
			...
			if(t.data.length==0) continue; //the case where there's only a label. Say after zooming in.
			...
			//populate all entries with NaN, which is what we want in the gaps.
	...
	public void showCollapsedData(ArrayLabelGenerator alg)
	{
        ...
        /**
        * When the contained graph has a range selection event, we want to translate it into the 
        * coordinates that our listeners are expecting and then dispatch it to them.
        */
	...
	/**
         * If we're displaying collapsed data then this does the translation back
         * from the collapsed range to the original range.
	 */
	...
	private final class ExpandedTransformer implements TSRangeTransformer
	{
		...
		{
			int i=TSChart.this.expansionArray.length-1;
			while (i>0 && TSChart.this.expansionArray[i]>x) i--;
			return TSChart.this.expansionArrayStart+i;
		}
		public int right(int x)
		{
			int i=0;
			while (i<TSChart.this.expansionArray.length-1 && TSChart.this.expansionArray[i]<x) i++;
			return TSChart.this.expansionArrayStart+i;
		}
	}
