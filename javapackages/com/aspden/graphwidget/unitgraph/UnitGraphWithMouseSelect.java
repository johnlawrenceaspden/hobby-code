 ...
 *Single clicks with the left button are signalled as rectangles with identical corners.
 */
public class UnitGraphWithMouseSelect extends UnitGraph
{
	...
	protected void processMouseEvent(MouseEvent e)
	{
		...
		if(e.getID()==MouseEvent.MOUSE_PRESSED)
		{
			//System.out.println("Mouse pressed");
		    	mouseCoordsAreShowing=true;
			pointerOverWindow=true;
			if((e.getModifiers()==MouseEvent.BUTTON1_MASK) && selectionAllowed)
			{
		...
		if(e.getID()==MouseEvent.MOUSE_RELEASED)
		{
			...
			if((e.getModifiers()==MouseEvent.BUTTON1_MASK) && selectionInProgress)
			{
				...
				fireRangeSelectionEvent(selectX,selectY,e.getX(),e.getY());
                        ...
                        {
                            fireRightButtonReleasedEvent(e.getX(),e.getY());
                        }
		}
	...
	private void fireRightButtonReleasedEvent(int x, int y)
	{
		Enumeration e=rangeListeners.elements();
		while(e.hasMoreElements())
		{
			UnitGraphRangeSelectionListener rsl=(UnitGraphRangeSelectionListener) e.nextElement();
			rsl.rightButtonReleased(xCoord(x),yCoord(y));
		}
	}
	...
	private void drawMouseLabel(Graphics g, int pointerx, int pointery, int displayx, int displayy)
	{
		if(theMouseLabelGenerator!=null)
		{
			...
			(new BoxObjectWrapper(new TextBox(s))).directedDraw(g,displayx,displayy,new Compass(Compass.SW));
		}
	}
