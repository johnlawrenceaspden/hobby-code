...
public class ScatterGraphPanel extends GraphPanel
{
	...
	protected void showSubClassData()
	{
		...
		if(xRangeExists) theGraph.addElement(new UnitGraphFloatingPointAxis(Compass.S, lGrid()||rGrid(), xMin, xMax));
		if(yLRangeExists) theGraph.addElement(new UnitGraphFloatingPointAxis(Compass.W, lGrid(), yLMin, yLMax));
		if(yRRangeExists)theGraph.addElement(new UnitGraphFloatingPointAxis(Compass.E, rGrid(), yRMin, yRMax));
		...
		while( de.hasMoreElements())
		{
			...
			if(b)
			{
				...
				{
					theGraph.addElement(a.getUnitGraphPoints(xMin, xMax, yLMin, yLMax));
				}
			...
			else
			{
				...
				{
					theGraph.addElement(a.getUnitGraphPoints(xMin, xMax, yRMin, yRMax));
				}
			}
		}
	}
	...
	public String getMouseLabel(double x,double y)
	{
		...
		if(yLRangeExists)
		{
			...
			label=label + SpectralClass.getCoordText(yLMax-yLMin,yc) + " <- " ;
		} 
		
		if(xRangeExists)
		{
			double xc=x*(xMax-xMin)+xMin;
			label=label + SpectralClass.getCoordText(xMax-xMin,xc);
		}
		
		if(yRRangeExists)
		{
			double yc=y*(yRMax-yRMin)+yRMin;
			label=label + " -> " + SpectralClass.getCoordText(yRMax-yRMin,yc)  ;
		}
