...
public class SampleTimeSeriesPanel extends Panel implements ArrayRangeSelectionListener, ItemListener, ActionListener
{
	...
	public SampleTimeSeriesPanel()
	{
		...
		chartA.addUnitGraphLabel(new UnitGraphLabel(Compass.NW,"Raw Data"));
		chartB.addUnitGraphLabel(new UnitGraphLabel(Compass.NW,"Expanded Data"));
		chartC.addUnitGraphLabel(new UnitGraphLabel(Compass.NW,"Collapsed Data"));
	...
	static private class Weekday implements ArrayLabelGenerator
	{
		...
		private static DateFormat df = DateFormat.getDateInstance(DateFormat.FULL);
  		...
  		{
			Date d=new Date(101, 0, wtod.getNewIndex(x)+1);
			return df.format(d);
  		}
			
  		public String getMaximalLabel()
  		{
			Date d=new Date(101, 0, 3);
			return df.format(d);
  		}
  }
  ...
  private final static class Day implements ArrayLabelGenerator
  {
		private static DateFormat df = DateFormat.getDateInstance(DateFormat.FULL);
  		...
  		public String getMaximalLabel()
  		{
			...
			Date d=new Date(101, 8, 19, 12, 0, 0); //Wednesday the 19th September 2001
