...
;frames are mutable structures containing a mutable list of mutable pairs of variables and values.

(define-struct sframe (bindings) #:mutable) ; add #:transparent to see everything

(define (make-empty-frame) (make-sframe '()))

(define (frame-bindings frame) (sframe-bindings frame))


(define (list-bindings frame)
  (let maploop ((bindings (frame-bindings frame)))
    (if (null? bindings) '()
      (cons (mcar (mcar bindings)) (maploop (mcdr bindings))))))

(define (environment->list-of-bindings env)
  (cond ((eq? env the-empty-environment) '())
        ;((eq? (enclosing-environment env) the-empty-environment) '(base-frame))
        (else (cons (list-bindings (first-frame env)) (environment->list-of-bindings (enclosing-environment env))))))

(define (add-binding-to-frame! var val frame)
  (let ((bindings (frame-bindings frame))
        (new-binding (mcons var val)))
    (set-sframe-bindings! frame (mcons new-binding bindings))))
