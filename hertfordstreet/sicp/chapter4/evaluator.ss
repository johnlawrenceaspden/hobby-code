...
#| alternative version using pattern matching (plt specific)
(define (eval exp env)
  (match exp
    ('true #t)
    ('false #f)
    ...
    ((? number? n) n)
    ((? symbol? exp) (lookup exp env))
    ((list 'define symbol value) (define-variable! symbol (eval value env) env))
    ((list 'quote exp) exp)
    ((or (list 'λ params body) 
         (list 'lambda params body)) (list 'closure (list params body) env))
    ((list-rest 'cond  sequence) (evcond  sequence env))
    ((list-rest 'begin sequence) (evbegin sequence env))
    ((list-rest function args) (sapply (eval function env) (evlist args env) env))
    ))|#
