...
(define (list-of-values exps env)
  (if (no-operands? exps)
      ...
      (cons a b)))) ;frigged to eval right-to-left to see if I'll notice
      ...
      (let ((first (car clauses))
        ...
        (if (cond-else-clause? first)
            (if (null? rest)
                ...
                (merror "ELSE clause isn't last -- COND->IF ~a"
                       clauses))
...
(define (extend-environment vars vals base-env)
  (if (= (length vars) (length vals))
      ...
      (if (< (length vars) (length vals))
          (merror "Too many arguments supplied vars: ~a vals: ~a" vars vals)
          (merror "Too few  arguments supplied vars: ~a vals: ~a" vars vals))))
  ...
  (define (env-loop env)
    (define (scan vars vals)
      (cond ((null? vars)
            ...
            (else (scan (mcdr vars) (mcdr vals)))))
    (if (eq? env the-empty-environment)
        (merror "Unbound variable -- SET! var: ~a" var)
        (let ((frame (first-frame env)))
          (scan (frame-variables frame)
                (frame-values frame)))))
  (env-loop env))
...
(printf 
"note that the tests for this module are in mevaltests.ss .
To get code coverage results, run the following
(require scheme/enter)
(enter! \"mevaltests.ss\")
or to run them by hand use the enter form at the REPL here
")

(printf "entering mevaltests in program to get code coverage (won't add bindings to repl):~n")
(require scheme/enter)
(enter! "mevaltests.ss")

(define the-global-environment (setup-environment))
(untrace meval)
(define-syntax-rule (me exp)
  (meval 'exp the-global-environment))
